...
const PROJECTS = [
  ...
  },
  {
    ...
    name: 'Helium',
    underDev: true,
    tagline: 'Autonomous Self-Healing Infrastructure Agent',
    description: 'An AI-powered infrastructure agent that monitors production systems in real time, detects anomalies using machine learning, diagnoses root causes using large language model reasoning, and autonomously remediates failures — all without human intervention in the critical path.',
    fullDescription: [
      'Helium is an AI-powered infrastructure agent designed to eliminate human latency in incident response. By monitoring production systems in real-time, it reduces the industry average Mean Time to Resolution (MTTR) from 12–15 minutes down to under 90 seconds — a 73% reduction across tested failure scenarios.',
      'The system operates on a sophisticated four-layer architecture. It continuously ingests metrics, structured logs, and distributed traces. An Anomaly Detection Engine processes this telemetry using a dual-model approach: an Isolation Forest to catch sudden point anomalies, and a PyTorch-based LSTM Autoencoder to identify subtle temporal degradation over time. A Correlation Engine then groups these diverse signals into a single incident cluster to drastically reduce alert noise.',
      'At its core, a LangGraph-based multi-agent system takes action. An Analyst agent performs Large Language Model (LLM) Root Cause Analysis (RCA), a Planner formulates a remediation strategy, and an Executor securely applies the fix to live infrastructure. The execution is governed by a strict Safety Layer that limits the blast radius, provides a dry-run mode, and maintains tamper-proof audit logs.',
      'What truly differentiates Helium is its Incident Memory System. Every resolved incident is stored as a vector embedding in ChromaDB. When new anomalies occur, Helium retrieves context from similar past incidents, allowing its diagnostic precision to continuously learn and improve over the lifetime of a deployment.'
    ],
    features: [
      'Autonomous Remediation',
      'Human-in-the-Loop Mode',
      'Safety Layer Limits & Audits',
